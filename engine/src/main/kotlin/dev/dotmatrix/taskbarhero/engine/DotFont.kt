...
/**
 * A 5x7 dot-matrix face, in the spirit of Nothing OS' Ndot: the renderer draws
 * one circle per lit cell instead of a filled glyph, which is what gives the
 * widget its perforated look.
 *
...
object DotFont {
    ...
    /** Cell of a laid-out string, in dot units relative to the text origin. */
    ...
    /** Width of [text] in dot units, including inter-letter [spacing]. */
     ...
     * The renderer just turns each [Dot] into a circle.
     */
    fun dots(text: String, spacing: Int = 1): List<Dot> {
        val out = ArrayList<Dot>(text.length * 12)
        var ox = 0
        for (c in text) {
            val g = glyph(c)
    ...
    /** Truncates to fit [maxDots] wide, appending nothing — the bar has no room for ellipses. */
    fun clip(text: String, maxDots: Int, spacing: Int = 1): String {
        if (measure(text, spacing) <= maxDots) return text
        var n = text.length
        while (n > 0 && measure(text.take(n), spacing) > maxDots) n--
        return text.take(n)
    }
     ...
     */
    fun clipWords(text: String, maxDots: Int, spacing: Int = 1): String {
        if (measure(text, spacing) <= maxDots) return text
        val words = text.split(' ')
        var out = ""
        for (word in words) {
            val candidate = if (out.isEmpty()) word else "$out $word"
            if (measure(candidate, spacing) > maxDots) break
            out = candidate
        }
        // Trailing separators read as truncation artefacts once the tail is gone.
        val trimmed = out.trimEnd(' ', '/', '-', ',')
        return if (trimmed.isEmpty()) clip(text, maxDots, spacing) else trimmed
    }
}
