...
import dev.dotmatrix.taskbarhero.paint.Palette
...
fun main(args: Array<String>) {
    ...
    val frames = listOf(
        ...
        Frame("bar-4x1-boss", 4, 1, state("boss"), "BOSS 3 DOWN / GLYPH CUBE SHARD"),
        Frame("bar-4x1-down", 4, 1, state("down"), null),
        Frame("bar-4x1-deep", 4, 1, state("deep"), null),
        Frame("bar-4x2", 4, 2, state("idled"), null),
        Frame("bar-5x2", 5, 2, state("deep"), "LEVEL 240"),
    )
...
private data class Frame(
    ...
    /** Roughly what a Nothing OS home-screen cell measures out to, in dp. */
...
private fun renderBar(frame: Frame): BufferedImage {
    ...
    BarLayout.draw(DotPainter(AwtSurface(g), unit), cols, rows, frame.state, T0, frame.event, BALANCE)
    g.dispose()
    return image
}
...
private fun renderDungeon(state: GameState): BufferedImage {
    ...
    val image = BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB)
    val g = image.createGraphics()
    g.color = java.awt.Color(Palette.BG, true)
    g.fillRect(0, 0, w, h)
    DungeonLayout.draw(
        p = DotPainter(AwtSurface(g), unit),
        cols = DungeonLayout.TARGET_COLS,
        rows = (h / unit).toInt(),
        state = state,
        log = listOf(
            "RUNE 14 +28%", "LEVEL 63", "BOSS 4 DOWN / GLYPH GLYPH LAMP",
            "ACT 4 CLEARED", "WIPED AT 5-08",
        ),
        nowMs = T0,
        b = BALANCE,
    )
...
private fun renderSheet(frames: List<Frame>): BufferedImage {
    ...
    g.color = java.awt.Color(0xFF0A0A0A.toInt(), true)
    g.fillRect(0, 0, width, height)

    var y = margin
    for ((frame, bar) in bars) {
        val painter = DotPainter(AwtSurface(g), 3.4f)
        val label = "${frame.cellsWide}X${frame.cellsTall} ${frame.name.substringAfterLast('-').uppercase()}"
        // Dot-font labels, drawn with a translated surface so the painter keeps
        // working in its own dot space.
        g.translate(margin, y)
        painter.text(0f, 0f, label, Palette.MID)
        g.translate(-margin, -y)
        y += labelHeight
