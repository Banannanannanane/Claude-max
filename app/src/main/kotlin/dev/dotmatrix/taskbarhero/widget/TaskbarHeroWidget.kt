package dev.dotmatrix.taskbarhero.widget
...
import dev.dotmatrix.taskbarhero.render.WidgetRenderer
import dev.dotmatrix.taskbarhero.ui.DungeonActivity

/**
 * The taskbar itself. Redraws are cheap and stateless: the engine derives the
 * whole fight from elapsed time, so a refresh once a minute is enough for the
 * bar to always show the truth — including after hours of a dark screen.
 */
class TaskbarHeroWidget : AppWidgetProvider() {
    ...
    private fun render(ctx: Context, manager: AppWidgetManager, id: Int) {
        ...
        val bitmap = WidgetRenderer.render(
            ...
            nowMs = now,
            recentEvent = GameStore.recentEvent(ctx, now, BarLayout.EVENT_FLASH_MS),
            b = GameStore.balance,
        )
    ...
    companion object {
        const val ACTION_TICK = "dev.dotmatrix.taskbarhero.TICK"
        const val ACTION_LEVEL_UP = "dev.dotmatrix.taskbarhero.LEVEL_UP"
