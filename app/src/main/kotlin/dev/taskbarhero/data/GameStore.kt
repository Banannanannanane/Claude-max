...
import dev.taskbarhero.engine.TickResult
...
object GameStore {
    ...
    private const val KEY_EVENT_TONE = "event_tone"
    private const val KEY_EVENT_GRADE = "event_grade"
    ...
    /**
     * The most recent event line, or null once it has gone stale. Tone and loot
     * grade travel with it: the widget draws the caption in the colour the event
     * earned, and by the next broadcast the event object itself is long gone.
     */
    fun recentEvent(ctx: Context, nowMs: Long, windowMs: Long): Ticker? {
        val p = prefs(ctx)
        val at = p.getLong(KEY_EVENT_AT, 0L)
        if (at == 0L || nowMs - at > windowMs || nowMs < at) return null
        val text = p.getString(KEY_EVENT_TEXT, null) ?: return null
        val tone = Tone.entries.getOrElse(p.getInt(KEY_EVENT_TONE, 0)) { Tone.NEUTRAL }
        return Ticker(text, tone, p.getInt(KEY_EVENT_GRADE, 0))
    }
    ...
    private fun save(p: SharedPreferences, s: GameState, event: GameEvent?, nowMs: Long) {
        p.edit().apply {
            ...
            if (event != null) {
                ...
                putInt(KEY_EVENT_TONE, event.tone.ordinal)
                putInt(KEY_EVENT_GRADE, event.grade)
